...
const sr = 48000, dur = 4.0, f = 210.0;
const n = Math.floor(sr * dur);
const buf = Buffer.alloc(44 + n * 2);
let o = 0;
const s = (str) => { buf.write(str, o); o += str.length; };
const u32 = (v) => { buf.writeUInt32LE(v, o); o += 4; };
const u16 = (v) => { buf.writeUInt16LE(v, o); o += 2; };
s('RIFF'); u32(36 + n * 2); s('WAVE'); s('fmt '); u32(16); u16(1); u16(1); u32(sr); u32(sr * 2); u16(2); u16(16);
s('data'); u32(n * 2);
for (let i = 0; i < n; i++) {
  const t = i / sr;
  let v = 0.6 * Math.sin(2 * Math.PI * f * t) + 0.25 * Math.sin(2 * Math.PI * 2 * f * t) + 0.12 * Math.sin(2 * Math.PI * 3 * f * t);
  v *= 0.5;
  buf.writeInt16LE(Math.max(-32768, Math.min(32767, Math.round(v * 32767))), o); o += 2;
}
